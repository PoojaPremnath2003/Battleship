...
#to establish connectivity between matplotlib and tkinter
...
def signin_entry():
    global status,signinscreen,signinpic,button2,entry1,entry2
    status='signin'
    canvas2.pack_forget()
...
#establishing Python-MySQL Connectivity to check validity of details entered
def db():
...
def grid():
   ...
   global f,status,signupscreen,signinscreen,origin,canvas4,pic,aircraftcarrier,battleship,submarine,cruiser,destroyer,d,squares,gameover
   origin='game'
   conn=mc.connect(host='localhost',user='root',password='pooja',database='x')
   cur=conn.cursor()
...
def mousepos(event,arg):
    conn=mc.connect(host='localhost',user='root',password='pooja',database='x')
    cur=conn.cursor()
    global hitdict,f,squares,white,red,hit,miss,shipdict
    if gameover==True:
        return
...
#a dictionary of coordinates available on the grid
...
#to reinitialize varibles to either 0 or to their empty state, to play once again
def consolation():
    global hitdict,squares,gameover,red,white,hit,miss,occupied,shipdict
    gameover=True
    canvas4.pack_forget()
...
#to draw a table to display the top scorers from the database    
def leaderboard():
